...
class FluxNetworkTrainer(train_network.NetworkTrainer):
    ...
    def load_target_model(self, args, weight_dtype, accelerator):
        ...
        # 强制使用 float8_e5m2 格式
        if args.fp8_base:
            if model.dtype != torch.float8_e5m2:
                model = model.to(torch.float8_e5m2)
                logger.info(f"Converted unet to float8_e5m2 for training.")
            else:
                logger.info(f"Loaded unet with float8_e5m2 format.")
                
            # 添加验证日志
            logger.info(f"UNet final dtype: {model.dtype}")
        ...
        if args.fp8_base and not args.fp8_base_unet:
            # 仅保留对e5m2格式的支持
            if t5xxl.dtype == torch.float8_e5m2:
                logger.info(f"Loaded fp8 T5XXL model with dtype: {t5xxl.dtype}")
            else:
                logger.warning(f"Unexpected T5XXL dtype: {t5xxl.dtype}")
    ...
    def prepare_split_model(self, model, weight_dtype, accelerator):
        ...
        # 对可训练部分也使用 e5m2
        if args.fp8_base_unet:
            flux_lower.to(dtype=torch.float8_e5m2)
            logger.info("Set flux_lower to float8_e5m2 for training.")
        else:
        ...
        if args.fp8_base_unet:
            ...
            logger.info("Forcing flux_upper to float8_e5m2 format.")
        else:
            target_dtype = model.dtype if args.fp8_base else weight_dtype
            
        flux_upper.to(accelerator.device, dtype=target_dtype)
        flux_upper.eval()
        
        # 添加验证日志
        logger.info(f"Flux_upper dtype: {flux_upper.dtype}")
    ...
    def cache_text_encoder_outputs_if_needed(
        ...
        if args.cache_text_encoder_outputs:
            ...
            # 修改点4: 支持 float8_e5m2
            if text_encoders[1].dtype in [torch.float8_e4m3fn, torch.float8_e5m2]:
    ...
    def prepare_text_encoder_fp8(self, index, text_encoder, te_weight_dtype, weight_dtype):
        if index == 0:  # CLIP-L
            ...
            # 修改点5: 支持 float8_e5m2
            actual_dtype = flux_utils.get_t5xxl_actual_dtype(text_encoder)
            if actual_dtype in [torch.float8_e4m3fn, torch.float8_e5m2] and text_encoder.dtype == weight_dtype:
